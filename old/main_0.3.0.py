# StepMania-Launcher 0.3.0
# Added downloading files via downloads.list
import os
import subprocess
...
def read_download_list(file_path="downloads.list"):
    ...
    with open(file_path, "r") as f:
        for line in f:
            line = line.strip()
            if line and ':' in line and line.startswith('[') and ']' in line:
                name_part = line[1:line.index(']')]
                url_part = line[line.index(':') + 1:].strip()
                downloads[name_part] = url_part
    return downloads
...
def main():
    ...
    # --- Download Section ---
    ttk.Label(root, text="Download a New Version:").pack(pady=10)

    downloads = read_download_list()
    if not downloads:
        ttk.Label(root, text="No downloads found in downloads.list").pack()
    else:
        selected_download = tk.StringVar()
        download_menu = ttk.Combobox(root, textvariable=selected_download, values=list(downloads.keys()), state="readonly")
        ...
        ttk.Button(root, text="Download!", command=start_download).pack(pady=10)
